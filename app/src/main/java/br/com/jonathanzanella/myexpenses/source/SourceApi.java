    ...
    public void save(UnsyncModel model) {
        ...
        try {
            ...
            if(response.isSuccessful()) {
	            ...
	            Log.Companion.info(LOG_TAG, "Updated: " + source.getData());
            } else {
                Log.Companion.error(LOG_TAG, "Save request error: " + response.message() + " uuid: " + source.getUuid());
            }
        } catch (IOException e) {
            Log.Companion.error(LOG_TAG, "Save request error: " + e.getMessage() + " uuid: " + source.getUuid());
            e.printStackTrace();
        }
    }
